...
import cn.harry12800.vchat.components.message.RCRightAttachmentMessageBubble;
...
public class MessageRightAttachmentViewHolder extends MessageAttachmentViewHolder {
	...
	private void initView() {
		...
		attachmentPanel.add(attachmentTitle,
				new GBC(1, 0).setWeight(100, 1).setAnchor(GBC.NORTH).setInsets(5, 5, 0, 5));
		...
		attachmentPanel.add(sizeLabel,
				new GBC(1, 1).setWeight(1, 1).setFill(GBC.HORIZONTAL).setAnchor(GBC.SOUTH).setInsets(-20, 8, 0, 0));
		...
		messageAvatarPanel.add(resendAttachmentPanel,
